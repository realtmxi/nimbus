"""Step-2 hybrid router: external FIFO queue + work-conserving dispatcher.

Architecture (agreed 2026-07-02):
...
Consequences:
  - no pressure  -> queue is always empty -> behavior == the open-loop Step-1
    router (this is the QUEUE-NEUTRALITY acceptance check);
...
the request's trace arrival time, comparable with the open-loop runner.
