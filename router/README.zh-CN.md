...
1. **可信源优先**:凡是能抄已验证 baseline 的代码,逐行抄、不重新发明。版本锚点:
   **`vllm/run.py` @ `dff1a81`**(Jialu 分支 mtp 版,她实际在跑的那份,GPU 机上
   md5 核对过;注意 main 上的旧版不同——有 `temperature`/`top_p`、无
   `stream_options`,对照实验须用 `dff1a81` 版)
2. **第一性原理**:不预建层。KV 感知、云延迟建模、nimbus 算法都有意排除(见"有意不做的事")
